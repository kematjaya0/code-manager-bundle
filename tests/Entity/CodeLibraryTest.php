...
class CodeLibraryTest extends TestCase
{
    ...
    public function testAttributes()
    {
        ...
        $attribute = $this->createMock(CodeLibraryAttribute::class);
        $attribute->expects($this->any())->method('setCodeLibrary')->willReturnSelf();
        $attribute->method('getCodeLibrary')->willReturn($entity);

        $entity->addCodeLibraryAttribute($attribute);
        $this->assertCount(1, $entity->getCodeLibraryAttributes());

        $entity->removeCodeLibraryAttribute($attribute);
        $this->assertCount(0, $entity->getCodeLibraryAttributes());
    }
}
